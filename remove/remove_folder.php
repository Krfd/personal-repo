            ...
            if ($filter_files->rowCount() > 0 || $filter_folder->rowCount() > 0) {
                ...
                while ($second_folder = $filter_folder->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if ($check_desc_files->rowCount() > 0) {
                        ...
                        $remove_desc_files = $conn->prepare("UPDATE file_upload SET is_deleted = 1, deleted_parent_folder = 1 WHERE asc_folder = '$folder'");
                        $remove_desc_files->execute();

                        // Removing child folder
                        $remove_child_folder = $conn->prepare("UPDATE folder_upload SET is_deleted = 1, deleted_parent_folder = 1 WHERE parent_folder = '$folder'");
                        ...
                        $remove_folder = $conn->prepare("UPDATE folder_upload SET is_deleted = 1 WHERE id = '$id'");
                        $remove_folder->execute();
                    }
                }

                // Removing child files
                $remove_child_files = $conn->prepare("UPDATE file_upload SET is_deleted = 1, deleted_parent_folder = 1 WHERE parent_folder = '$folder'");
