...
$files = $conn->prepare("SELECT DISTINCT files.category, categories.category FROM file_upload AS files LEFT JOIN category AS categories ON files.category = categories.category WHERE files.client_id = :client_id AND files.is_archive = 0 AND files.is_deleted = 1 GROUP BY categories.category");
$files->bindParam(':client_id', $client_id);
$files->execute();

$folder = $conn->prepare("SELECT DISTINCT folders.category, categories.category FROM folder_upload AS folders LEFT JOIN category AS categories ON folders.category = categories.category WHERE folders.client_id = :client_id AND folders.is_archive = 0 AND folders.is_deleted = 1 GROUP BY categories.category;");
...
WHERE files.client_id = :client_id AND folders.client_id = :client_id AND files.parent_folder = '' 
AND folders.parent_folder = '' AND files.is_archive = 0 AND folders.is_archive = 0 AND files.is_deleted = 1 
AND folders.is_deleted = 1 GROUP BY categories.category");
