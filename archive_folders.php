                ...
                <div class="container-fluid mx-auto">
                    <div class="header">
                        ...
                        if ($folder->rowCount() > 0) {
                            foreach ($folder as $folderupload) {
                                ...
                                echo '<h1 class="header font-weight-semibold"><a href="archive.php" class="text-secondary header">Archive</a> <i class="fa-solid fa-angle-right small"></i> ' . $folderCategory . ' -   ' . $folderName . '</h1>';
                            }
                        }
                        ?>
                    </div>
                    <div class="my-4">
                        ...
                        <div class="d-block d-md-flex d-md-flex-row justify-content-start flex-wrap" id="file-container">
                            ...
                            // Get the folder path
                            $folder = $conn->prepare("SELECT folder FROM folder_upload WHERE folder_upload.client_id = :client_id AND folder = :folder AND is_deleted = 0 AND is_archive = 1 ORDER BY id DESC");
                            $folder->bindParam(':client_id', $client_id);
                            ...
                            if ($folder->rowCount() > 0) {
                                foreach ($folder as $folderupload) {
                                    ...
                                    $folder = $conn->prepare("SELECT * FROM folder_upload WHERE folder_upload.client_id = :client_id AND parent_folder = '$mainFolder' ORDER BY id DESC");
                                    $folder->bindParam(':client_id', $client_id);
                                    $folder->execute();
                                            ...
                                            '<ul class="list-group list-unstyled" id="myFolder">
                                                <li class="list group" data-category="' . $folderupload['category'] . '">
                                                    <div class="col card shadow-sm p-2 my-1 my-md-3 mr-0 mr-md-1 mr-lg-3 d-flex" id="cardFile" style="max-width: 210px; min-width: 210px; min-height: 210px; max-height: 280px;" data-toggle="tooltip" title="Category: ' . $folderupload['category'] . '">
                                                        <a href="archive_inner_folder.php?parent_path=' . $parent_path . '&path=' . $folder_path . '" class="d-block mx-auto display-1 p-3 imageContainer">
                                                        ...
                                                        <div class="mt-auto px-1">
                                                            ...
                                                            <div class="d-flex justify-content-between">
                                                                ...
                                                                <div class="dropdown-menu">
                                                                    <a href="archive_inner_folder.php?parent_path=' . $parent_path . '&path=' . $folder_path . '"  class="dropdown-item"><i class="fa-regular fa-folder-open"></i> Open</a>
                    ...
                    <div class="my-4">
                        ...
                        <div class="d-block d-md-flex d-md-flex-row justify-content-start flex-wrap" id="file-container">
                            ...
                            // Get the folder path
                            $folder = $conn->prepare("SELECT folder FROM folder_upload WHERE folder_upload.client_id = :client_id AND folder = :folder AND is_deleted = 0 AND is_archive = 1");
                            $folder->bindParam(':client_id', $client_id);
                            $folder->bindParam(':folder', $mainFolder);
                            $folder->execute();
                            ...
                            if ($folder->rowCount() > 0) {
                                foreach ($folder as $folder_upload) {
                                    ...
                                    $file = $conn->prepare("SELECT * FROM file_upload WHERE file_upload.client_id = :client_id AND folder = '$folderPath' AND is_deleted = 0 AND is_archive = 1");
                                    ...
                                    if ($file->rowCount() > 0) {
                                        foreach ($file as $fileupload) {
                                            ...
                                            $asc_folder = $fileupload['asc_folder'];
                                            $modified = date('M. d, y', strtotime($fileupload['updated']));

                                            $filePath = '';
                                            if ($parent_folder != '' && $asc_folder != '') {
                                                $filePath = 'folders/' . $asc_folder . '/' . $parent_folder . "/" . $fileupload['fileName'];
                                            } else if ($parent_folder != '') {
                                                $filePath = 'folders/' . $parent_folder . "/" . $fileupload['fileName'];
                                            } else {
                                            ...
                                            // Variable to return to the current page after an action
                                            $page = "archive_folder.php?path=$folderPath";
                                            ...
                                            <ul class="list-group" id="myList" >
                                                <li class="list-group" data-category="' . $fileupload['category'] . '">
                                                    <div class="col card shadow-sm p-2 my-1 my-md-3 mr-0 mr-md-1 mr-lg-3 d-flex" id="cardFile" style="max-width: 210px; min-width: 210px; min-height: 210px; max-height: 210px;">
                                                        <a href=' . $filePath . ' target="_blank" class="imageContainer" style="overflow: hidden" style="position:relative">';
                                            // Conditional Rendering for different file formats
                                            $extension = pathinfo($fileupload['fileName'], PATHINFO_EXTENSION);
                                            switch ($extension) {
                                                case "jpg":
                                                    echo '<img src=' . $filePath . ' class="object-fit-cover h-auto w-100 btn p-0 rounded-0" alt=' . $fileupload['title'] . ' style="min-height: 210px;">';
                                                    break;
                                                case "png":
                                                    echo '<img src=' . $filePath . ' class="object-fit-cover h-auto w-100 btn p-0 rounded-0" alt=' . $fileupload['title'] . ' style="min-height: 210px;">';
                                                    break;
                                            ...
                                            <small>' . $modified . '</small>
                                                        <div class="d-flex justify-content-between cardName" ">
                                                            <span class="overflow-hidden pr-3 cardName">' . $fileName . '</span>
                                                                <i class="fa-solid fa-ellipsis-vertical dropdown rounded-circle py-1 px-2 align-self-center" data-toggle="dropdown" type="button" id="toggleMenuOption"></i>
                                                                <div class="dropdown-menu">
                                                                    <a href=' . $filePath . ' class="dropdown-item" target="_blank"><i class="fa-regular fa-folder-open" target="_blank"></i> Open</a>
                                                                    ...
                                                                    <a href=' . $filePath . ' class="dropdown-item" download><i class="fa-solid fa-download"></i> Download</a>
                                                                </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <form action="pdoaction.php" method="POST" class="user">
                                                                <input type="hidden" name="return" value="<?php echo "archive_folders.php?path=" . $mainFolder ?>">
