    ...
    <div id="wrapper" style="height: 100vh;">
        ...
        <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <li class="nav-item link bg-light m-1 rounded">
                <a class="nav-link text-secondary" href="trash.php">
                ...
                <div class="container-fluid mx-auto">
                    <div class="header">
                        ...
                        if ($folder->rowCount() > 0) {
                            foreach ($folder as $folderupload) {
                                ...
                                echo '<h1 class="header font-weight-semibold"><a href="trash.php" class="text-secondary header">Trash</a> <i class="fa-solid fa-angle-right small"></i> <a href="trash_folders.php?path=' . $parentFolder . '" class="text-secondary header">' . $parentFolder . '</a> <i class="fa-solid fa-angle-right small"></i> ' . $folderCategory . ' - ' . $folderName . '</h1>';
                            }
                        }
                        ?>
                    </div>
                    <div class="my-4">
                        ...
                        <div class="d-block d-md-flex d-md-flex-row justify-content-start flex-wrap" id="file-container">
                            ...
                            // Get the folder path
                            $folder = $conn->prepare("SELECT folder FROM folder_upload WHERE folder_upload.client_id = :client_id AND folder = :folder AND is_deleted = 1 ORDER BY id DESC");
                            $folder->bindParam(':client_id', $client_id);
                            ...
                            if ($folder->rowCount() > 0) {
                                foreach ($folder as $folder_upload) {
                                    ...
                                    $file = $conn->prepare("SELECT * FROM file_upload WHERE file_upload.client_id = :client_id AND folder = '$mainFolder' AND is_deleted = 1 ORDER BY file_id DESC");
                                    ...
                                    if ($file->rowCount() > 0) {
                                        foreach ($file as $fileupload) {
                                            ...
                                            $page = "storage_inner_folder.php?parent_path=$parentPath" . "&" . "path=$mainFolder";

                                            echo '
                            <ul class="list-group" id="myList">
                                <li class="list-group" data-category="' . $fileupload['category'] . '">
                                    <div class="col card shadow-sm p-2 my-1 my-md-3 mr-0 mr-md-1 mr-lg-3 d-flex" id="cardFile" style="max-width: 210px; min-width: 210px; min-height: 210px; max-height: 280px;">
                                        <a disabled class="imageContainer" style="overflow: hidden" style="position:relative" data-toggle="tooltip" title="Category: ' . $fileupload['category'] . '">';
